...
def widget_js():
    ...
    "// inject minimal widget CSS (scoped classes)\n"
    "var __cw_css = '.cb-btn{position:fixed;bottom:24px;width:56px;height:56px;padding:8px;border-radius:12px;border:none;background:transparent;display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:99999;box-shadow:0 8px 30px rgba(2,6,23,.08);transition:transform .12s ease,box-shadow .12s ease,background .12s;backdrop-filter: blur(6px);} .cb-btn:hover{transform:translateY(-4px);box-shadow:0 18px 44px rgba(2,6,23,.12);background:rgba(255,255,255,.7);} .cb-btn svg{width:32px;height:32px;display:block;} .cb-badge{position:absolute;top:-6px;right:-6px;min-width:24px;height:18px;padding:0 6px;border-radius:12px;display:none;align-items:center;justify-content:center;font-size:12px;line-height:18px;background:#2563eb;color:#fff;font-weight:700;box-shadow:0 6px 18px rgba(2,6,23,.12);z-index:999999;} .cb-badge .dot{width:5px;height:5px;border-radius:50%;background:#fff;display:inline-block;margin:0 2px;opacity:.25;transform:translateY(0);animation:badge-dot 1s infinite;} .cb-badge .dot:nth-child(2){animation-delay:.12s;} .cb-badge .dot:nth-child(3){animation-delay:.24s;} @keyframes badge-dot{0%{transform:translateY(0);opacity:.25}50%{transform:translateY(-5px);opacity:1}100%{transform:translateY(0);opacity:.25}} .cb-panel{position:fixed;bottom:96px;width:380px;max-width:calc(100vw - 32px);border-radius:14px;overflow:hidden;display:none;flex-direction:column;z-index:99998;box-shadow:0 30px 80px rgba(2,6,23,.18);background:linear-gradient(180deg,rgba(255,255,255,0.98),rgba(250,250,252,0.98));border:1px solid rgba(16,24,40,0.04);transform-origin:right bottom;} .cb-head{display:flex;align-items:center;justify-content:space-between;padding:14px 16px;border-bottom:1px solid rgba(2,6,23,.04);} .cb-title{font-weight:700;} .cb-body{height:340px;overflow:auto;padding:14px;display:flex;flex-direction:column;gap:10px;background:transparent;} .cb-input{display:flex;gap:10px;padding:12px;border-top:1px solid rgba(2,6,23,.04);background:transparent;} .cb-input input{flex:1;padding:10px 12px;border-radius:10px;border:1px solid rgba(2,6,23,.06);outline:none;} .cb-send{padding:10px 14px;border-radius:10px;border:none;background:#2563eb;color:#fff;cursor:pointer;} .bubble{max-width:80%;padding:10px 12px;border-radius:14px;line-height:1.45;font-size:14px;word-break:break-word;box-shadow:0 6px 18px rgba(15,23,42,.03);} .bubble.me{margin-left:auto;background:linear-gradient(180deg,#2563eb,#1e40af);color:#fff;border-bottom-right-radius:6px;} .bubble.bot{margin-right:auto;background:#fff;color:#0f1724;border:1px solid rgba(2,6,23,.04);border-bottom-left-radius:6px;} .typing{display:inline-flex;align-items:flex-end;gap:6px;padding:6px 10px;border-radius:12px;} .typing .dot{width:8px;height:8px;border-radius:50%;background:rgba(2,6,23,.2);animation:dot 1s infinite;} .typing .dot:nth-child(2){animation-delay:.12s;} .typing .dot:nth-child(3){animation-delay:.24s;} @keyframes dot{0%{transform:translateY(0);opacity:.25}50%{transform:translateY(-6px);opacity:1}100%{transform:translateY(0);opacity:.25}}';\n"
    "try{var __cw_style=document.createElement('style');__cw_style.innerHTML=__cw_css;document.head.appendChild(__cw_style);}catch(_){}\n"
        ...
        "function setButtonTyping(on){if(!SHOW_BADGE){return;}try{var btn=null;var btns=document.querySelectorAll('button');for(var ii=0;ii<btns.length;ii++){var el=btns[ii];try{if(el&&el.style&&(el.style.borderRadius==='999px'||(el.textContent||'').indexOf(N||'Chat')!==-1)){btn=el;break;}}catch(_){}}if(!btn){return;}if(!btn.__cw_badge){var badge=document.createElement('span');badge.className='cb-badge';badge.innerHTML='<span class=\"dot\"></span><span class=\"dot\"></span><span class=\"dot\"></span>';try{btn.style.position='relative';btn.appendChild(badge);}catch(_){}btn.__cw_badge=badge;}if(on){btn.__cw_badge.style.display='inline-flex';}else{btn.__cw_badge.style.display='none';}}catch(_){}}\n"
        "var __original_send=send;send=function(m,onchunk){try{setButtonTyping(true);}catch(_){}__original_send(m,function(tok,end){if(end){try{setButtonTyping(false);}catch(_){}}onchunk(tok,end);});}\n"
