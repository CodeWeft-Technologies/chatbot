...
def booking_form(bot_id: str, org_id: str, bot_key: Optional[str] = None):
    html = (
        ...
        "</div>"
        "<script>"
        "const ORG='" + org_id + "',BOT='" + bot_id + "',BOT_KEY='" + (bot_key or '') + "';let chosen=null,loading=false;"
        ...
        "async function loadReq(){try{const h={};if(BOT_KEY)h['X-Bot-Key']=BOT_KEY;const r=await fetch('/api/bots/'+BOT+'/booking/settings?org_id='+ORG,{headers:h});const d=await r.json();window.REQ=d.required_user_fields||[];}catch(e){console.error('loadReq error:',e);window.REQ=[];}}"
