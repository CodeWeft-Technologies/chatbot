...
def get_embed_snippet(bot_id: str, org_id: str, widget: str = "bubble", authorization: Optional[str] = Header(default=None), x_bot_key: Optional[str] = Header(default=None)):
    ...
    try:
        ...
        def cdn():
            js = f"<script>window.chatbotConfig={{botId:'{bot_id}',orgId:'{org_id}',apiBase:'{base}',botKey:'{key or ''}'}};</script><script src='{base}/api/widget.js' async></script>"
            return js
        def bubble():
            return (
                "<script>"
...
def widget_js():
    ...
    js = (
        "(function(){\n"
        "var C=window.chatbotConfig||{};var B=C.botId,O=C.orgId;var A=C.apiBase||'"+base+"';var K=C.botKey||null;var T='"+theme+"';var W=C.welcome||'';\n"
        "if(!B||!O){return;}\n"
        "var busy=false;\n"
        ...
        "function add(parent,type,text){var row=document.createElement('div');row.style.display='flex';row.style.margin='8px 0';row.style.justifyContent=type==='me'?'flex-end':'flex-start';var b=document.createElement('div');b.style.maxWidth='80%';b.style.padding='10px 12px';b.style.borderRadius='14px';b.style.lineHeight='1.5';b.style.fontSize='14px';b.style.whiteSpace='normal';if(type==='me'){b.style.background=T==='dark'?'#1f2937':'#2563eb';b.style.color='#fff';b.style.borderBottomRightRadius='6px';}else{b.style.background=T==='dark'?'#111827':'#fff';b.style.color=T==='dark'?'#1f2937':'#111827';b.style.border='1px solid '+(T==='dark'?'#374151':'#e5e7eb');b.style.borderBottomLeftRadius='6px';}if(text){b.innerHTML=md(text);}row.appendChild(b);parent.appendChild(row);parent.scrollTop=parent.scrollHeight;return b;}\n"
        "function ui(){var w=document.createElement('div');w.style.position='fixed';w.style.bottom='24px';w.style.right='24px';w.style.zIndex='99999';var b=document.createElement('button');b.textContent='Chat';b.style.padding='12px 16px';b.style.borderRadius='999px';b.style.border='none';b.style.boxShadow='0 8px 24px rgba(0,0,0,0.18)';b.style.background=T==='dark'?'#334155':'#2563eb';b.style.color='#fff';w.appendChild(b);var p=document.createElement('div');p.style.position='fixed';p.style.bottom='80px';p.style.right='24px';p.style.width='380px';p.style.maxWidth='90vw';p.style.background=T==='dark'?'#0b1220':'#fff';p.style.border='1px solid '+(T==='dark'?'#1f2937':'#e5e7eb');p.style.boxShadow='0 18px 40px rgba(0,0,0,0.20)';p.style.borderRadius='14px';p.style.display='none';p.style.overflow='hidden';var hd=document.createElement('div');hd.style.display='flex';hd.style.alignItems='center';hd.style.justifyContent='space-between';hd.style.padding='12px 14px';hd.style.borderBottom='1px solid '+(T==='dark'?'#1f2937':'#e5e7eb');var ttl=document.createElement('div');ttl.textContent='Chatbot';ttl.style.fontWeight='600';ttl.style.color=T==='dark'?'#e5e7eb':'#111827';var x=document.createElement('button');x.textContent='×';x.style.border='none';x.style.background='transparent';x.style.fontSize='18px';x.style.color=T==='dark'?'#94a3b8':'#6b7280';hd.appendChild(ttl);hd.appendChild(x);var a=document.createElement('div');a.style.height='320px';a.style.overflow='auto';a.style.padding='12px';a.style.background=T==='dark'?'#0b1220':'#fafafa';var inr=document.createElement('div');inr.style.display='flex';inr.style.gap='8px';inr.style.padding='12px';inr.style.borderTop='1px solid '+(T==='dark'?'#1f2937':'#e5e7eb');inr.style.background=T==='dark'?'#0b1220':'#fff';var i=document.createElement('input');i.type='text';i.placeholder='Ask a question';i.style.flex='1';i.style.padding='10px 12px';i.style.border='1px solid '+(T==='dark'?'#1f2937':'#e5e7eb');i.style.borderRadius='10px';i.style.background=T==='dark'?'#0b1220':'#fff';i.style.color=T==='dark'?'#e5e7eb':'#111827';var go=document.createElement('button');go.textContent='Send';go.style.padding='10px 14px';go.style.border='none';go.style.borderRadius='10px';go.style.background=T==='dark'?'#334155':'#2563eb';go.style.color='#fff';inr.appendChild(i);inr.appendChild(go);p.appendChild(hd);p.appendChild(a);p.appendChild(inr);document.body.appendChild(w);document.body.appendChild(p);function open(){p.style.display='block';if(a.childNodes.length===0&&W){add(a,'bot',W);} }function close(){p.style.display='none';}b.onclick=function(){if(p.style.display==='none'){open();}else{close();}};x.onclick=close;function doSend(){if(busy){return;}var m=i.value.trim();if(!m){return;}busy=true;i.value='';i.disabled=true;go.disabled=true;add(a,'me',m);var bot=add(a,'bot','');var acc='';send(m,function(tok,end){if(end){busy=false;i.disabled=false;go.disabled=false;return;}acc+=tok;bot.innerHTML=md(acc);});}go.onclick=doSend;i.onkeydown=function(e){if((e.key||e.keyCode)==='Enter'||e.keyCode===13){e.preventDefault();doSend();}};}\n"
        "ui();\n"
        "})();\n"
    )
    return js
